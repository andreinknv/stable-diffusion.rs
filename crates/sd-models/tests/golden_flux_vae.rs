...
macro_rules! refs_or_skip {
    ($dev:expr) => {{
        let (r, w) = (golden("reference.safetensors"), golden("vae.safetensors"));
...
#[test]
fn flux_decoder_matches_diffusers() {
    let dev = Device::Cpu;
    let (refs, vb) = refs_or_skip!(&dev);

    let dec = AutoencoderKlDecoder::new(&VaeConfig::flux(), vb).unwrap();
    ...
    eprintln!(
        "flux decode: max_abs {:.3e}, mean_abs {:.3e}",
...
#[test]
fn flux_encoder_matches_diffusers() {
    let dev = Device::Cpu;
    let (refs, vb) = refs_or_skip!(&dev);

    let enc = AutoencoderKlEncoder::new(&VaeConfig::flux(), vb).unwrap();
    ...
    eprintln!(
        "flux encode: mean max_abs {:.3e}, logvar max_abs {:.3e}",
...
#[test]
fn flux_latent_scale_and_shift_round_trip() {
    let dev = Device::Cpu;
    let (refs, vb) = refs_or_skip!(&dev);
