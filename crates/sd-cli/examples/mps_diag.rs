...
fn candle_conv(xs: &Tensor, k: &Tensor, ci: usize, co: usize) -> Result<Tensor> {
    let vb = VarBuilder::from_tensors(
        [("weight".into(), k.clone())].into_iter().collect(),
        DType::F32,
        &xs.device().clone(),
    );
    let c = nn::conv2d_no_bias(
        ci,
        co,
        3,
        nn::Conv2dConfig {
...
fn main() -> Result<()> {
    ...
    for (name, t) in [
        ("candle OIHW", &same),
        ("candle transposed", &swapped),
        ("candle kernel-flipped", &flipped),
    ] {
        println!(
            "mps vs {name:<22} max_abs {:.3e}",
            sd_tensor::testing::max_abs_diff(&mps_out, t)?
        );
    }
    Ok(())
}
